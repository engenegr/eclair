...
import fr.acinq.eclair.{CltvExpiry, MilliSatoshiLong, TestConstants, TestKitBaseClass, randomKey}
...
class NegotiatingStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with StateTestsBase {
  ...
  def testClosingSigned(f: FixtureParam): Unit = {
    ...
    if (alice.stateData.asInstanceOf[DATA_NEGOTIATING].commitments.channelVersion.hasOptionUpfrontShutdownScript) {
