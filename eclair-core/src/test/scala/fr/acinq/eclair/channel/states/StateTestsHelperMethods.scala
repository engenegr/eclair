...
trait StateTestsHelperMethods extends TestKitBase {
  ...
  def reachNormal(setup: SetupFixture, tags: Set[String] = Set.empty): Unit = {
    ...
    val channelType = {
      val dummy = if (tags.contains(StateTestsTags.AnchorOutputs)) {
      ...
      if (tags.contains(StateTestsTags.OptionUpfrontShutdownScript)) {
        dummy.copy(features = dummy.features.copy(activated = dummy.features.activated + (Features.OptionUpfrontShutdownScript -> FeatureSupport.Optional)))
      } else dummy
    }
