...
object Helpers {
   ...
   */
  def validateParamsFundee(nodeParams: NodeParams, localFeatures: Features, remoteFeatures: Features, channelVersion: ChannelVersion, open: OpenChannel, remoteNodeId: PublicKey): Either[ChannelException, Option[ByteVector]] = {
    ...
    Try(extractShutdownScript(channelVersion.hasOptionUpfrontShutdownScript, allowAnySegwit, open.tlvStream.get[ChannelTlv.UpfrontShutdownScript])) match {
   ...
   */
  def validateParamsFunder(nodeParams: NodeParams, localFeatures: Features, remoteFeatures: Features, channelVersion: ChannelVersion, open: OpenChannel, accept: AcceptChannel): Either[ChannelException, Option[ByteVector]] = {
    ...
    val allowAnySegwit = Features.canUseFeature(localFeatures, remoteFeatures, Features.ShutdownAnySegwit)
    Try(extractShutdownScript(channelVersion.hasOptionUpfrontShutdownScript, allowAnySegwit, accept.tlvStream.get[ChannelTlv.UpfrontShutdownScript])) match {
      case Failure(_) => Left(InvalidFinalScript(open.temporaryChannelId))
      case Success(script) => Right(script)
    }
  }
  ...
  object Closing {
    ...
    def firstClosingFee(commitments: Commitments, localScriptPubkey: ByteVector, remoteScriptPubkey: ByteVector, feeratePerKw: FeeratePerKw)(implicit log: LoggingAdapter): Satoshi = {
      ...
      val actualLocalScript = if (channelVersion.hasOptionUpfrontShutdownScript) localParams.defaultFinalScriptPubKey else localScriptPubkey
      val actualRemoteScript = if (channelVersion.hasOptionUpfrontShutdownScript) remoteParams.shutdownScript.getOrElse(remoteScriptPubkey) else remoteScriptPubkey
