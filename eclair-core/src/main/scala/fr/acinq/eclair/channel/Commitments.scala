...
case class Commitments(channelVersion: ChannelVersion,
  ...
  def getLocalShutdownScript(scriptPubKey: Option[ByteVector]): Either[ChannelException, ByteVector] = {
    ...
    (channelVersion.hasOptionUpfrontShutdownScript, scriptPubKey) match {
  ...
  def getRemoteShutdownScript(remoteScriptPubKey: ByteVector): Either[ChannelException, ByteVector] = {
    val allowAnySegwit = Features.canUseFeature(localParams.features, remoteParams.features, Features.ShutdownAnySegwit)
    (channelVersion.hasOptionUpfrontShutdownScript, remoteParams.shutdownScript) match {
      ...
      case (true, None) if !Closing.isValidFinalScriptPubkey(remoteScriptPubKey, allowAnySegwit )=> Left(InvalidFinalScript(channelId))
